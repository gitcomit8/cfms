  ...
  return (
    <div className="max-w-5xl mx-auto px-4 py-8">
      <div className="flex justify-between items-center mb-6">
        <h1 className="text-2xl font-bold text-charcoal">Manage Venues</h1>
      ...
      {showForm && (
        <div className="bg-white rounded-xl border border-charcoal/20 p-6 mb-6">
          <div className="flex justify-between items-center mb-4">
            <h2 className="font-semibold text-charcoal">{editing ? 'Edit Venue' : 'Create Venue'}</h2>
          ...
          <form onSubmit={handleSubmit} className="grid grid-cols-2 gap-4">
            <div className="col-span-2">
              <label className="block text-xs font-medium text-charcoal/80 mb-1">Venue Name *</label>
            ...
            <div>
              <label className="block text-xs font-medium text-charcoal/80 mb-1">Capacity</label>
              <input type="number" name="capacity" value={form.capacity} onChange={handleChange} min="1"
                className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-grape/80" />
            </div>
            <div>
              <label className="block text-xs font-medium text-charcoal/80 mb-1">Street</label>
              <input name="street" value={form.street} onChange={handleChange}
                className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-grape/80" />
            </div>
            <div>
              <label className="block text-xs font-medium text-charcoal/80 mb-1">City</label>
              <input name="city" value={form.city} onChange={handleChange}
                className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-grape/80" />
            </div>
            <div>
              <label className="block text-xs font-medium text-charcoal/80 mb-1">State</label>
              <input name="state" value={form.state} onChange={handleChange}
                className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-grape/80" />
            </div>
            <div>
              <label className="block text-xs font-medium text-charcoal/80 mb-1">ZIP</label>
